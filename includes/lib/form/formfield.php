...
use tp\lib\Layout\FileLayout;
use tp\lib\Utilities\Registry;
use tp\lib\Language\TPLanguage;
use tp\lib\Utilities\ArrayHelper;
use tp\lib\Filter\InputFilter;

if ( ! class_exists( 'tp\lib\Form\FormField' ) ) {
    abstract class FormField
    {
        ...
        protected function getTitle()
        {
            ...
            $title = $this->translateLabel ? TPLanguage::_($title) : $title;
        ...
        public function validate($value, $group = null, Registry $input = null)
        {
            ...
            if ($required && (($value === '') || ($value === null)))
            {
                ...
                {
                    $message = TPLanguage::_($this->element['label']);
                }
                else
                {
                    $message = TPLanguage::_($this->element['name']);
                }

                $message = TPLanguage::sprintf('JLIB_FORM_VALIDATE_FIELD_REQUIRED', $message);
            ...
            if ($valid === false)
            {
                ...
                if ($message)
                {
                    $message = TPLanguage::_($this->element['message']);
                }
                else
                {
                    $message = TPLanguage::_($this->element['label']);
                    $message = TPLanguage::sprintf('JLIB_FORM_VALIDATE_FIELD_INVALID', $message);
                }
        ...
        protected function getLayoutData()
        {
            ...
            $label = $this->translateLabel ? TPLanguage::_($label) : $label;

            // Description preprocess
            $description = !empty($this->description) ? $this->description : null;
            $description = !empty($description) && $this->translateDescription ? TPLanguage::_($description) : $description;
            ...
            return array(
                ...
                'group'          => $this->group,
                'hidden'         => $this->hidden,
                'hint'           => $this->translateHint ? TPLanguage::alt($this->hint, $alt) : $this->hint,
                'id'             => $this->id,
                'label'          => $label,
